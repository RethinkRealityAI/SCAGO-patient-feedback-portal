...
function buildZodSchema(fields: FieldDef[], requiredOverrides: Set<string>) {
  ...
  return base.superRefine((values: Record<string, any>, ctx) => {
    ...
    for (const f of fields) {
      ...
      if ((f as any).otherOption?.enabled && (f as any).otherOption?.required && (f as any).otherOption?.optionValue) {
        const triggerValue = (f as any).otherOption.optionValue;
    ...
    <div className="flex items-center space-x-1">
      {[...Array(5)].map((_, index) => {
        ...
        return (
          <Star
            key={ratingValue}
            className={`h-8 w-8 cursor-pointer transition-colors ${ratingValue <= (hover || field.value || 0)
                ...
                <div className="max-h-60 overflow-y-auto">
                  {filteredOptions.length === 0 ? (
                    ...
                    filteredOptions.map((option) => (
                      ...
                      >
                        <Check
                          className={`mr-2 h-4 w-4 ${selection === option.value ? "opacity-100" : "opacity-0"
                            }`}
                        />
                        {translateOption(option.label, isFrench ? 'fr' : 'en')}
                      </div>
                      ...
                      <div className="space-y-2">
                        ...
                        {(fieldConfig as any).otherOption?.enabled && (
                          <FormItem className="flex flex-row items-start space-x-3 space-y-0">
                            <FormControl>
                              <Checkbox
                                checked={field.value?.includes((fieldConfig as any).otherOption?.optionValue || 'other')}
                                onCheckedChange={(checked) => {
                                  const otherValue = (fieldConfig as any).otherOption?.optionValue || 'other';
                                  const currentValue = field.value || [];
                                  if (checked) {
                                    field.onChange([...currentValue, otherValue]);
                                  } else {
                                    field.onChange(currentValue.filter((v: string) => v !== otherValue));
                                  }
                                }}
                              />
                            </FormControl>
                  ...
                  <CardContent className="px-4 sm:px-6 lg:px-8 mt-6">
                    <div className="space-y-8">
                      ...
                      {(section.fields || []).map((field: FieldDef) => {
                        ...
                        const hasOtherOption = (fieldDef as any).otherOption?.enabled &&
                          (fieldDef as any).otherOption?.optionValue &&
                        ...
                        const showOtherField = hasOtherOption && (() => {
                          ...
                          const triggerValue = (fieldDef as any).otherOption.optionValue;
                                ...
                                <FormField
                                  ...
                                  render={({ field: otherField }) => (
                                    <FormItem>
                                      <FormLabel>
                                        {translateFieldLabel(
                                          (fieldDef as any).otherOption?.label || 'Please specify',
                                          isFrench ? 'fr' : 'en'
                                        )}
                                      </FormLabel>
                                      <FormControl>
                                        {(fieldDef as any).otherOption.fieldType === 'textarea' ? (
                                          <Textarea
                                            {...otherField}
