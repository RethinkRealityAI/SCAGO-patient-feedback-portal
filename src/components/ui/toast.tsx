  ...
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-lg border p-6 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full",
  {
    variants: {
      variant: {
        ...
        success:
          "border-emerald-200 bg-emerald-50 text-emerald-900 shadow-emerald-500/50 shadow-lg dark:border-emerald-800 dark:bg-emerald-950 dark:text-emerald-100",
        destructive:
          "destructive group border-red-200 bg-red-50 text-red-900 shadow-red-500/50 shadow-lg dark:border-red-800 dark:bg-red-950 dark:text-red-100",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), variant === "success" && "success", className)}
      {...props}
    />
  )
})
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ...
    className={cn(
      ...
      "group-[.destructive]:text-red-600 group-[.destructive]:hover:text-red-900 group-[.destructive]:focus:ring-red-400",
      "group-[.success]:text-emerald-600 group-[.success]:hover:text-emerald-900 group-[.success]:focus:ring-emerald-400",
      className
    )}
    toast-close=""
    {...props}
