...
import { upsertParticipantByEmail } from './actions';
import { hashSIN, extractSINLast4 } from '@/lib/youth-empowerment';
...
function calculateDOB(age: number | null): string {
...
function mapCitizenshipStatus(status: string): 'Canadian Citizen' | 'Permanent Resident' | 'Other' {
  if (!status) return 'Other';
  
  const statusMap: Record<string, 'Canadian Citizen' | 'Permanent Resident' | 'Other'> = {
...
function isUnderage(age: number | null): boolean {
  return age !== null && age < 18;
}

// Main import function
export async function importCurrentParticipants(): Promise<{ success: boolean; message: string; imported: number; skipped: number }> {
  ...
  try {
    for (const participant of currentParticipantsData) {
      ...
      // Use upsertParticipantByEmail to handle existing participants
      // This will update existing participants or create new ones
      ...
      const participantData = {
        ...
        etransferEmailAddress: '', // Not available in current data
        mailingAddress: '', // Not available in current data
        phoneNumber: participant.phone || '',
        region: region,
        approved: participant.recruited || false,
        contractSigned: false,
        signedSyllabus: false,
        availability: '', // Not available in current data
        assignedMentor: '', // Not available in current data
        idProvided: false,
        canadianStatus: canadianStatus,
        canadianStatusOther: canadianStatus === 'Other' ? participant.citizenshipStatus : '',
        sinLast4: '', // Not available in current data
        sinHash: '', // Not available in current data
        youthProposal: participant.duties || '',
        proofOfAffiliationWithSCD: participant.affiliationWithSCD ? 
          participant.affiliationWithSCD.includes('SCD') || participant.affiliationWithSCD.includes('Sickle Cell') : false,
      ...
      // Use upsertParticipantByEmail to create or update participant
      const result = await upsertParticipantByEmail(participantData);
      
      if (result.success) {
        console.log(`Imported/Updated ${participant.name}`);
        imported++;
      } else {
        console.log(`Failed to import ${participant.name}: ${result.error}`);
        skipped++;
      }
    }
    
    console.log(`Import completed successfully! Imported: ${imported}, Skipped: ${skipped}`);
    return {
      success: true,
      message: `Successfully imported ${imported} participants. Skipped ${skipped} participants (already exist or no email).`,
      imported,
      skipped
    };
  } catch (error) {
